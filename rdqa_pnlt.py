...
def load_data():
    """Charge les données BCZS et CDT depuis le repository GitHub"""
    
    df_bczs = pd.DataFrame()
    df_cdt = pd.DataFrame()
    
    # Chemins dans votre repository GitHub (dossier data/)
    paths_to_try = [
        # Convention standard GitHub - dossier data/
        "data/drc_pnlt_rdqa_bczs_WIDE.csv",
        "data/drc_pnlt_rdqa_cdt_WIDE.csv",
        # Autres possibilités
        "./data/drc_pnlt_rdqa_bczs_WIDE.csv",
        "./data/drc_pnlt_rdqa_cdt_WIDE.csv",
        # Racine du projet
        "drc_pnlt_rdqa_bczs_WIDE.csv",
        "drc_pnlt_rdqa_cdt_WIDE.csv",
    ]
    
    # 1. CHARGER LES DONNÉES BCZS
    bczs_loaded = False
    for bczs_path in paths_to_try:
        if "bczs" in bczs_path.lower() and os.path.exists(bczs_path):
            try:
    ...
    # 2. CHARGER LES DONNÉES CDT
    cdt_loaded = False
    for cdt_path in paths_to_try:
        if "cdt" in cdt_path.lower() and os.path.exists(cdt_path):
            try:
                df_cdt = pd.read_csv(cdt_path, encoding='utf-8')
    ...
    # 3. AFFICHER LES RÉSULTATS DU CHARGEMENT
    if bczs_loaded:
        st.success(f"✅ Données BCZS chargées: {len(df_bczs)} Zones de Santé")
    else:
        st.error("❌ Fichier BCZS non trouvé. Assurez-vous que le fichier existe dans le dossier 'data/'")
        st.info("""
        **Pour résoudre ce problème:**
        1. Vérifiez que votre fichier s'appelle `drc_pnlt_rdqa_bczs_WIDE.csv`
        2. Placez-le dans le dossier `data/` de votre repository
        3. Ou téléchargez-le via l'interface ci-dessous
        """)
    
    if cdt_loaded:
        st.success(f"✅ Données CDT chargées: {len(df_cdt)} Centres")
    else:
        st.error("❌ Fichier CDT non trouvé. Assurez-vous que le fichier existe dans le dossier 'data/'")
        st.info("""
        **Pour résoudre ce problème:**
        1. Vérifiez que votre fichier s'appelle `drc_pnlt_rdqa_cdt_WIDE.csv`
        2. Placez-le dans le dossier `data/` de votre repository
        3. Ou téléchargez-le via l'interface ci-dessous
        """)
    
    # 4. OPTION DE TÉLÉCHARGEMENT SI LES FICHIERS NE SONT PAS TROUVÉS
    if not bczs_loaded or not cdt_loaded:
        st.markdown("---")
        st.markdown("### 📤 TÉLÉCHARGEMENT MANUEL DES DONNÉES")
        
        col1, col2 = st.columns(2)
        
        with col1:
            uploaded_bczs = st.file_uploader(
                "Télécharger données BCZS (CSV):",
                type=['csv'],
                key="bczs_upload",
                help="Fichier CSV avec les données BCZS"
            )
            
            if uploaded_bczs is not None:
                try:
                    df_bczs = pd.read_csv(uploaded_bczs, encoding='utf-8')
                    st.session_state.df_bczs = df_bczs
                    st.success(f"✅ BCZS téléchargées: {len(df_bczs)} ZS")
                    bczs_loaded = True
                except Exception as e:
                    st.error(f"Erreur de chargement BCZS: {str(e)}")
        
        with col2:
            uploaded_cdt = st.file_uploader(
                "Télécharger données CDT (CSV):",
                type=['csv'],
                key="cdt_upload",
                help="Fichier CSV avec les données CDT"
            )
            
            if uploaded_cdt is not None:
                try:
                    df_cdt = pd.read_csv(uploaded_cdt, encoding='utf-8')
                    st.session_state.df_cdt = df_cdt
                    st.success(f"✅ CDT téléchargés: {len(df_cdt)} centres")
                    cdt_loaded = True
                except Exception as e:
                    st.error(f"Erreur de chargement CDT: {str(e)}")
        
        # Recharger la page si des données sont téléchargées
        if (uploaded_bczs is not None or uploaded_cdt is not None) and (bczs_loaded or cdt_loaded):
            st.rerun()
...
def main():
    ...
    # ============================================================================
    if 'df_bczs' not in st.session_state or 'df_cdt' not in st.session_state:
        with st.spinner('Chargement des données depuis le repository...'):
    ...
    # ============================================================================
    with st.sidebar:
        st.markdown("## 🎯 NAVIGATION PAR PROVINCE")
        st.markdown("---")
        ...
        if selected_province != all_provinces_option:
            ...
            if zones_in_province:
                ...
                selected_zone = st.selectbox(
                    ...
                    zones_list,
                    key="zone_selector",
                    help="Filtrer les données par zone de santé"
                )
    ...
    # ============================================================================
    # FILTRE CONTEXTUEL - Afficher en haut de la page
    # ============================================================================
    if selected_province != all_provinces_option:
        filter_text = f"**📍 Filtre actuel :** {selected_province}"
        if selected_zone != "Toutes les zones":
